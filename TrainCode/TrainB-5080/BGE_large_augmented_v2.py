"""
Track B训练 - BGE (无数据泄露版本)
只使用训练集信息,dev set完全独立
"""
import os
...
def main():
    print("🚀 Track B训练 - BGE (无数据泄露版本)")
    print("✅ 只使用训练集信息优化,dev set完全独立\n")
    print("=" * 60)
    ...
    OUTPUT_PATH = f'{PROJECT_ROOT}/output/track_b_bge_optimized_no_leak_5080'
    ...
    # 🌟 关键: 使用无泄露版本的数据
    OPTIMIZED_TRAIN_DATA = f'{PROJECT_ROOT}/TrainingSet_optimized_no_leak/augmented_training_data_no_leak.jsonl'
    ...
    # 1. 优化后的训练数据 (只从训练集挖掘困难样本)
    print(f"1. 加载优化后的训练数据 (无泄露版本)...")
    ...
    # === 评估器 (dev set完全独立,从未用于训练) ===
    evaluator = TrackB_Accuracy_Evaluator_NoSave(
        name="bge_optimized_no_leak",
    ...
    print(f"   数据: 无泄露版本 (只在训练集上找困难样本)")
    print(f"   总样本: {len(train_dataset):,}")
    print(f"   Batch size: {BATCH_SIZE}")
    print(f"   Learning rate: {LEARNING_RATE}")
    print(f"   Epochs: {EPOCHS}")
    print(f"   预期提升: 0.66 → 0.68-0.70 (真实性能)")
    ...
    # === 训练完成总结 ===
    print(f"\n{'=' * 60}")
    print(f"✅ 训练完成 (无数据泄露版本)!")
    print(f"{'=' * 60}")
    print(f"\n📊 实验对比:")
    print(f"   Baseline (原始数据): 0.66")
    print(f"   有泄露版本: 0.72 (不可靠)")
    print(f"   无泄露版本: 待测试 (预期 0.68-0.70)")
    print(f"\n💡 关键区别:")
    print(f"   ✅ 困难样本来自训练集 (不是dev set)")
    print(f"   ✅ dev set完全独立,从未用于训练")
    print(f"   ✅ 性能是真实可靠的")
    print(f"\n📁 模型位置:")
